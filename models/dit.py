...
#################################################################################
# Dit for text diffusion without patch embedding
#################################################################################
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y=None, use_cross_attention = True, **unused):
        """
        Forward pass of DiT.
...
##################################################
# DiT for image diffusion with patch embeddings
...
class DiT2(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiTBlock2(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
        self.final_layer = FinalLayer2(hidden_size, patch_size, self.out_channels)
        self.initialize_weights()
...
def DiT_B_4(**kwargs):
    return DiT2(depth=12, hidden_size=768, patch_size=4, num_heads=12, **kwargs)